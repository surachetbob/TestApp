...
with st.expander("👤 ข้อมูลผู้ขอเอาประกัน", expanded=True):
    ...
    calculate_submit = st.button("🚀 คำนวณเบี้ยประกัน")

# ใช้ Session State เพื่อจำว่าคำนวณแล้วหรือยัง
if calculate_submit or 'calculated' in st.session_state:
    ...
    st.markdown(f"""
        <div class="premium-box">
            ...
            <p style="color:#555;">แผน: {plan_choice} | อายุ: {age} ปี</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    # 2. ส่วน "สนใจ" (จะปรากฏหลังจากคำนวณแล้ว)
    st.divider()
    st.subheader("🎯 สนใจให้เจ้าหน้าที่ติดต่อกลับ?")
    
    with st.form(key="lead_form"):
        name = st.text_input("ชื่อ-นามสกุล", placeholder="กรุณากรอกชื่อ")
        phone = st.text_input("เบอร์โทรศัพท์", placeholder="08XXXXXXXX", max_chars=10)
        interest_submit = st.form_submit_button("✅ ส่งข้อมูลให้เจ้าหน้าที่ติดต่อกลับ")

        if interest_submit:
            if not name or len(phone) < 10 or not phone.isdigit():
                st.error("❌ กรุณากรอกชื่อและเบอร์โทรศัพท์ให้ถูกต้อง")
            else:
                try:
                    ...
                    st.success(f"ขอบคุณครับคุณ {name} เจ้าหน้าที่จะติดต่อกลับที่เบอร์ {phone} โดยเร็วที่สุด")
                    st.balloons()
                except Exception as e:
                    st.error(f"เกิดข้อผิดพลาดในการบันทึกข้อมูล: {e}")

    st.info("📞 **ติดต่อด่วน:** มีน โทร 086-5348448")

else:
    st.info("💡 กรุณากรอกข้อมูลและกดปุ่ม 'คำนวณเบี้ยประกัน' เพื่อดูรายละเอียด")
